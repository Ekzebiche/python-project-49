...
def progression_game() -> tuple[str, str]:
    start = utils.number_gen(1, 10)
    step = utils.number_gen(1, 10)
    count = utils.number_gen(5, 15)
    progression = list(range(start, start + step * count, step))

    result = str(utils.random_choose(progression))
...
def start():
    utils.game_starter(progression_game)
